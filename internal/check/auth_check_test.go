...
func TestAuthCheck(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			client: http.DefaultClient,
			expErr: check.ErrInvalidExpectedStatus,
		},
		{
			name: "invalid url",
			cfg: config.AuthCheck{
		...
		},
		{
			...
			client: nil,
			expErr: check.ErrNilClient,
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			c, err := check.NewAuth(test.cfg, test.client)
...
func SetupAuthCheck_Execute(token string, client *http.Client, cfg config.AuthCheck) (*check.Auth, *httptest.Server, *http.Request, context.Context, *string, *string) {
...
func TestAuthCheck_Execute_Unauthorized(t *testing.T) {
	...
	newCtx, err := c.Execute(ctx, req)
	if !errors.Is(err, check.ErrUnauthorized) {
		t.Fatalf("expected error %v, but got %v", check.ErrNilRequest, err)
	}
	if v := newCtx.Value("token"); v != nil {
		t.Fatalf("expected in context empty token, but got %s", v)
...
func TestAuthCheck_Execute_Success_EmptyBody(t *testing.T) {
	...
	cfg.URL = srv.URL
	c, _ := check.NewAuth(cfg, http.DefaultClient)
