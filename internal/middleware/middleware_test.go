...
func TestInject(t *testing.T) {
	...
	n := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if v := r.Context().Value("name"); v != "niceName" {
			t.Fatalf("expected context value for key 'name' - 'niceName', but got %v", v)
		}

		if v := r.Context().Value("id"); v != 1001 {
			t.Fatalf("expected context value for key 'id' - '1001', but got %v", v)
		}
	})

	m := Inject(map[string]any{
		"name": "niceName",
		"id":   1001,
	})
	...
	if !called {
		t.Fatal("expected next handler to be called, but it is not")
	}
